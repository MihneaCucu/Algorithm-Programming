...
for proiect in lp:
    for z in range(proiect[1], 0, -1):
        if planificare[z] is None:
            ...
            break
planificare2 = {}
for proiect in planificare:
    if planificare[proiect]:
        planificare2[proiect] = planificare[proiect]
planificare2 = {index+1: proiect for index,proiect in enumerate(planificare2.values())}

# enumerate returneaza o lista de tupluri
# scriem soluția în fișierul text "proiecte.out"

fout = open("proiecte.out", "w")
for z in planificare:
    if planificare[z] != None:
        fout.write("Ziua " + str(z) + ": " + planificare[z][0] + " " + str(planificare[z][2]) + "\n")
